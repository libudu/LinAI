...
export const formatSettings = (settings: NovelArtifact[]): string =>
...
// 生成新文段（op: generate）：按产出类型拼一小段类型说明
export const buildGenerateTask = (
  outputType: ArtifactType,
  opts: {
    /** outline/content：章节序号 */
    index?: number
    /** content：目标篇幅（字） */
    targetLength?: number
    /** 用户附加要求（setting 的产出要求也走这里） */
    instruction?: string
    /** setting：本次上下文是否带了参考文 */
    hasRefs?: boolean
  } = {},
): string => {
  const { index = 1, targetLength = 3000, instruction, hasRefs } = opts
  switch (outputType) {
    case 'setting':
      return `【任务】
...
【任务】
只按修改指令调整，其余内容保持不变，输出修改后的完整${label}${type === 'outline' ? '（仍按原格式）' : ''}，不要输出任何说明。`
}

// 续写正文（op: continue），已有正文末尾作为前缀
...
// 重写选中段落（op: rewrite-range）
export const buildRewriteRangeTask = (
  content: string,
  selection: string,
  instruction: string,
): string =>
