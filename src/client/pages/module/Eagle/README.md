...
│   ├── StepConfirm.tsx                  # 步骤 3：纯净结果确认——仅查验判定成功项，顶部缩略图条 + 左大图（原图展示并预加载下两张）右信息面板 + 底部快捷操作（A清除分类/S不处理/重新执行/D确认），重新执行不打断确认流
...
9. 图片整理流程：
   ...
   - **步骤 3（待确认）**：纯净查验判定成功的结果（`status === 'success'`）；大图使用原图展示并自动预加载后续两张；支持快捷键（A清除分类/S不处理/D确认）；单图重新执行将该图重置为 pending 送回步骤 2 队列，步骤 3 自动聚焦下一张，不打断确认流。
