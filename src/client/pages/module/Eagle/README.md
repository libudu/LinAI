...
│   │   ├── ThumbnailBar.tsx             # 顶部缩略图滚动列表（右侧集成排序组件：完成顺序/图片分类/修改时间）
...
9. 图片整理流程：
   ...
   - **步骤 3（待确认）**：纯净查验判定成功的结果（`status === 'success'`）；顶部缩略图条右侧支持切换排序（完成顺序/图片分类/修改时间），大图使用原图展示并自动预加载后续两张；支持快捷操作（移到回收站/A清除分类/S不处理/D确认）；单图重新执行将该图重置为 pending 送回步骤 2 队列，步骤 3 自动聚焦下一张，不打断确认流。
